<script lang="ts">
    ...
    import type { qHistory } from "$lib/types";
    import GameBreakdown from "./GameBreakdown.svelte";
    import ShowHideToggle from "../ShowHideToggle.svelte";

    export let score: number;
    export let qHistory: qHistory;
    ...
    onMount(() => {
        ...
        if (localStorage.gamesCompleted) {
            localStorage.gamesCompleted =
                parseInt(localStorage.gamesCompleted) + 1;
        } else {
...
{#if highscore !== null}
    <div class="py-20 px-5" in:fly={{ y: 25, duration: 300, delay: 400 }}>
            ...
            {#if counterFinished}
                ...
                <div
                    in:fly={{
                        y: 10,
                        duration: 300,
                        delay: beatHighscore ? 1700 : 1000,
                    }}
                >
                    <ShowHideToggle text="Breakdown">
                        <GameBreakdown {qHistory} />
                    </ShowHideToggle>
                </div>
            {/if}
        </div>
